...
class Menu(Layer, KeyListener):
  def __init__(self, engine, choices, onClose = None, onCancel = None, pos = (.2, .66 - .35), viewSize = 6, fadeScreen = False, font = "font"):
    self.engine       = engine
    ...
    if pos == (.2, .66 - .35) and self.theme == 1:
      pos = (.44, .14)
    if viewSize == 6 and self.theme == 1:
      viewSize = 9
