...
class Song(object):
  def __init__(self, engine, infoFileName, songTrackName, guitarTrackName, rhythmTrackName, noteFileName, scriptFileName = None):
    ...
    self.difficulty   = difficulties[EXP_DIF]
    self._playing      = False
    self.start         = 0.0
...
class ScriptReader:
  ...
  def read(self):
#   for line in self.file.xreadlines(): # this (old) method will cause decoding errors on special chars
