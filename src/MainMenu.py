...
class MainMenu(BackgroundLayer):
  def __init__(self, engine, songName = None):
    ...
    self.engine.config.set("game", "pov", 0)
    self.engine.loadSvgDrawing(self, "background", "keyboard.png")
    self.engine.loadSvgDrawing(self, "guy",        "pose.png")
    self.engine.loadSvgDrawing(self, "logo",       "logo.png")
    self.song = Audio.Sound(self.engine.resource.fileName("menu.ogg"))

    self.song.setVolume(self.engine.config.get("audio", "songvol"))
    self.song.play(-1)

    # expire = datetime.date(2013, 4, 15) 
  ...
  def render(self, visibility, topMost):
    ...
    if self.theme == 0:
      self.engine.view.transitionTime = 512.0
    elif self.theme == 1:
